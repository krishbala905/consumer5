...
Future<String> callPPNAPIXML(BuildContext context , var action,var pnsId,var confirmStatus,var type)async {
  ...
  if (response.statusCode == 200) {
   ...
   if(p1.toLowerCase()=="true") {
     ...
     // consumer5
     else if(actionType==CommonUtils.KEY_VOUCHER_REDEEM) {
       showAlertDialog_oneBtn_msgg(context, msg);
       CommonUtils.pid=p3;
      // navigatePath=CommonUtils.walletPage;
...
print(data1.toString());
 ...
 /* print(imageUrl);
  ...
  Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => MemberTransaction(
      prgmId, "", totalAmount, totalAmount, "", merchantName, "", queue, transactionId, feedbackPoints, countryIndex, outletId, "1", "",pnsId
  ),));
  /*Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => MemberTransaction(
...
void showAlertDialog_oneBtn_msgg(BuildContext context,String message)
{
